...
static uint16_t lastTimeChecked = 0;
...
void displayUpdate(deviceStateInfo_t deviceState, uint16_t secondsElapsed)
{
    ...
    case DISPLAY_STEPS:
        ...
        // prompt to move
        if ((secondsElapsed - lastTimeChecked)>3)
        {
            if ((deviceState.stepsTaken - lastStepCount)<1)
            {
                displayLine("Please move!", 3, ALIGN_CENTRE);
            }else{
                displayLine("", 3, ALIGN_CENTRE); // Clear the bottom line
                lastStepCount = deviceState.stepsTaken;
                lastTimeChecked = secondsElapsed;
            }
        } else {
            displayLine("", 3, ALIGN_CENTRE); // Clear the bottom line
        }
    ...
    case DISPLAY_TEMPERATURE:
        displayLine("", 0, ALIGN_CENTRE); // Clear the top line
        displayLine("Temperature:", 1, ALIGN_CENTRE);
        displayValue("", "", deviceState.temp, 2, ALIGN_CENTRE, false);
        displayLine("", 3, ALIGN_CENTRE); // Clear the bottom line
        break;
    case DISPLAY_CALS:
        displayLine("", 0, ALIGN_CENTRE); // Clear the top line
        displayLine("Calories:", 1, ALIGN_CENTRE);
        displayValue("", "", deviceState.cals, 2, ALIGN_CENTRE, false);
        displayLine("", 3, ALIGN_CENTRE); // Clear the bottom line
        break;
    case DISPLAY_STOPWATCH_LAPS:
        displayLine("STOPWATCH: ", 0, ALIGN_CENTRE); 
        if (deviceState.paused) {
            displayTime("Time:", deviceState.workoutPausedVal, 1, ALIGN_CENTRE);
        } else {
            displayTime("Time:", secondsElapsed - abs(deviceState.workoutGoTick - deviceState.workoutPausedVal), 1, ALIGN_CENTRE);
        }
        displayLine("LAPS:", 2, ALIGN_CENTRE);
        displayValue("","", deviceState.laps, 3, ALIGN_CENTRE, false);
        break;
    }
}
